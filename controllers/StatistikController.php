...
class StatistikController extends BaseController
{
    public function actionIndex($method = 'minmax', $basis = 'perdept')
    {
        ...
        $base = (new Query())
            ...
            ->innerJoin(['per' => 'master_periode'], 'per.id_periode = p.id_periode'); // <— penting

        if ($katId) {
            $base->andWhere(['p.id_kategori' => $katId]);
        }

        if ($start) {
            $base->andWhere(['>=', 'per.tanggal_mulai', $start]);
        }
        if ($end) {
            $base->andWhere(['<=', 'per.tanggal_selesai', $end]);
        }

        $picked = array_filter([$deptA ?: null, $deptB ?: null], fn($v) => $v !== null);
        if (!empty($picked)) {
            $base->andWhere(['u.id_departement' => $picked]);
        }
